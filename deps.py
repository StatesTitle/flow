...
if __name__ == "__main__":
    ...
    notify_lender_of_scheduling = Vertex(
        "Notify Lender of Scheduling", [schedule_notary]
    )
    receive_closing_documents = Vertex(
        "Receive Closing Documents", [notify_lender_of_scheduling]
    )
    prepare_affidavits = Vertex(
        "Prepare Affidavits for Borrower", [receive_closing_documents]
    )
    prepare_fedex = Vertex("Prepare FedEx Label", [schedule_notary])
    prepare_notary_instructions = Vertex(
        "Prepare Notary Instructions", [prepare_affidavits]
    )
    ...
    notary_sends_scanned_package = Vertex(
        "Notary Emails Signed Package", [send_closing_to_notary]
    )
    notary_sends_physical_pacakge = Vertex(
        "Notary FedExes Signed Package", [send_closing_to_notary]
    )
    send_lender_closing = Vertex(
        "Send Lender Signed Package", [notary_sends_physical_pacakge]
    )
    lender_funds_loan = Vertex("Lender Funds Loan", [send_lender_closing])
    disbursement_arrives = Vertex(
        "Receive Disbursement sent via Wire", [lender_funds_loan]
    )
    documents_sent_to_recording = Vertex(
        "Send Documents to Recording", [disbursement_arrives]
    )
    create_title_policy = Vertex("Complete Title Policy", [disbursement_arrives])
    mail_title_policy_with_deed_to_lender = Vertex(
        "Mail Policy with Deed to Lender",
        [create_title_policy, documents_sent_to_recording],
    )
    ...
    for line in digraph(
        [
            mail_title_policy_with_deed_to_lender,
            mail_title_policy_with_remittance_to_underwriter,
